...
$email = $password = $fullname = $gender = $phone = $state = $country = $address = $farmLocation = $profilePicture = "";
$emailErr = $passwordErr = $fullnameErr = $genderErr = $phoneErr = $stateErr = $countryErr = $addressErr = $farmLocationErr = $profilePictureErr = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["email"])) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($db->read("vendors", "WHERE email='$email'")) {
    ...
    // If all validations pass, insert the vendor
    if ($isValid) {
        $status = 'active';
        $created_at = date('Y-m-d H:i:s');
        $hashedPassword = password_hash($password, PASSWORD_BCRYPT);

        // Insert vendor without profile picture
        $vendorId = $db->create("vendors", [
            'email' => $email,
        ...
        if ($vendorId) {
            // Rename the uploaded file to vendor ID
            $newFileName = $vendorId . '.jpg';
            $newDestPath = $uploadDir . $newFileName;

            if (rename($destPath, $newDestPath)) {
                // Update vendor with the new profile picture filename
                $db->update("vendors", ['image' => $newFileName], "id='$vendorId'");
            } else {
            ...
            $profilePictureErr = "There was an error creating the vendor.";
        }
    }
}
?>
...
<div class="content-wrapper">
  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Dashboard /</span> Add Vendor</h4>

    <div class="card mb-4">
      <h5 class="card-header">Add New Vendor
      ...
      <div class="card-body">
        ...
        <form method="POST" enctype="multipart/form-data">
          ...
          <div class="mt-2">
            <button type="submit" class="btn btn-primary me-2">Add Vendor</button>
          </div>
        </form>
      </div>
    </div>
